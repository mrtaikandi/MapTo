...
internal static class ScenarioBuilder
{
    ...
    public static ITestSourceBuilder SimpleMappedClassInSameNamespaceAsSource(TestSourceBuilderOptions? options = null, bool genericMapFromAttribute = false)
    {
        options ??= TestSourceBuilderOptions.Create();
        options.AddAnalyzerConfigOption(nameof(CodeGeneratorOptions.ProjectionType), ProjectionType.None.ToString());
        ...
        var attribute = genericMapFromAttribute
            ? "[MapTo.MapFrom<SourceClass>]"
            : "[MapFrom(typeof(SourceClass))]";

        sourceFile.AddClass(Accessibility.Public, "TargetClass", attributes: attribute).WithProperty<int>("Id").WithProperty<string>("Name");

        var generatedFile = builder.AddFile(ExpectedFileName, autoGenerated: true, usings: ["global::System.Linq"]);
